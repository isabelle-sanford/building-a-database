...
func (lm LogMgr) iterator() func() []byte {
	...
	recpos := int(lm.logPage.getInt(0))
	...
	return func() []byte {
		...
		if recpos >= lm.fm.blocksize { 
			...
			pg, _ = lm.fm.readBlock(BlockId{lm.logfile, blknum}, pg) // probs don't need pg, _ ? 
			recpos = int(pg.getInt(0))
		}
		lsn-- // ehhh
		ret := pg.getBytes(recpos)
		recpos += int(pg.getInt(recpos)) // ! text is different but I THINK this also works
